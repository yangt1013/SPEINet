...
class VIDEODATA(data.Dataset):
    def __init__(self, args, name='', train=True):
        ...
        self.images_gt, self.images_input = self._scan()   #, self.images_bm, self.images_label
        ...
        if args.process:
            self.data_gt, self.data_input = self._load(self.images_gt, self.images_input)   #, self.data_bm, self.data_label    , self.images_bm, self.images_label
    ...
    def _scan(self):
        ...
        assert len(vid_gt_names) == len(vid_input_names), "len(vid_gt_names) must equal len(vid_input_names)"  #== len(vid_bm_names) == len(vid_label_names)
        ...
        for vid_gt_name, vid_input_name in zip(vid_gt_names, vid_input_names):  #, vid_bm_name, vid_label_name    , vid_bm_names, vid_label_names
            if self.train:
    ...
    def __getitem__(self, idx):
        if self.args.process:
            inputs, gts, filenames = self._load_file_from_loaded_data(idx)   #, bms, labels
        else:
            inputs, gts, filenames = self._load_file(idx)   #, bms, labels
        ...
        return torch.stack(input_tensors), torch.stack(gt_tensors), filenames  #, torch.stack(bm_tensors), label_tensors
    ...
    def _load_file(self, idx):
        ...
        return inputs, gts, filenames   #, bms, f_labels
    ...
    def _load_file_from_loaded_data(self, idx):
        ...
        return inputs, gts, filenames   #, bms, labels
