...
class VIDEODATA(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            inputs_list = [inputs[i, :, :, :] for i in range(self.n_seq+2)] #self.n_seq=3, 
        except:
            print(self.n_seq, inputs.shape)
        ...
        inputs_list = [inputs_concat[:, :, i*self.args.n_colors:(i+1)*self.args.n_colors] for i in range(self.n_seq+2)]  #2
        ...
        # print('input_tensors', input_tensors[3])
    ...
    def _load_file(self, idx):
        ...
        # Check the frame number difference between the third image and the last two images
        ...
        # Check and replace images with frame number difference greater than 7
        if abs(frame_numbers[2] - frame_numbers[3]) > 7:
            inputs[-2] = 0  # Replace the tensor value of the fourth image with 0
        # if abs(frame_numbers[2] - frame_numbers[4]) > 7:
        #     inputs[-1] = 0  # Replace the tensor value of the fifth image with 0
        if inputs.shape[0] != 5:
            print(filenames)

        return inputs, gts, f_labels, filenames#, pre_s_index_list, sub_s_index_list
