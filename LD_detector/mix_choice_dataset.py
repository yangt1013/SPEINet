...
dir_path_val = '/home/yangt/ssd1/dataset/GOPRO_Large_all/val'
dir_path_test = '/home/yangt/ssd1/dataset/GOPRO_Large_all/test'
output_dir = '/home/yangt/ssd1/dataset/GOPRO_Large_all/newblur'
output_file = '/home/yangt/ssd1/dataset/mix_train_data.json'
...
def process_dataset(dataset, data_idx, mode):
    ...
    for i, (blurry_frame, gt_frame) in enumerate(zip(vid_input_names, vid_gt_names)):
        blurry_image_path = os.path.join(blur_subdir_path, f'{i}.png')
        gt_image_path = os.path.join(gt_subdir_path, f'{i}.png')
...
def save_output_to_file(blurry_paths_train, gt_paths_train, labels_train, blurry_paths_val, gt_paths_val, labels_val, blurry_paths_test, gt_paths_test, labels_test, output_file):
    output_data = {
        ...
        "Val Blurry Paths": blurry_paths_val,
        "Val GT Paths": gt_paths_val,
        "Val Labels": [label.tolist() for label in labels_val],  # Convert numpy arrays to lists
        "Test Blurry Paths": blurry_paths_test,
        "Test GT Paths": gt_paths_test,
        "Test Labels": [label.tolist() for label in labels_test]  # Convert numpy arrays to lists
    }
...
def generate_datasets():
    ...
    gopro_0_train = GoProrc(dir_path_train, 0.1)
    gopro_25_train = GoProrc(dir_path_train, 0.3)
    gopro_5_train = GoProrc(dir_path_train, 0.5)
    gopro_mix_train = [gopro_0_train, gopro_25_train, gopro_5_train]

    gopro_0_val = GoProrc(dir_path_val, 0.5)
    gopro_25_val = GoProrc(dir_path_val, 0.5)
    gopro_5_val = GoProrc(dir_path_val, 0.5)
    gopro_mix_val = [gopro_0_val, gopro_25_val, gopro_5_val]

    gopro_0_test = GoProrc(dir_path_test, 0.1)
    gopro_25_test = GoProrc(dir_path_test, 0.3)
    gopro_5_test = GoProrc(dir_path_test, 0.5)
    gopro_mix_test = [gopro_0_test, gopro_25_test, gopro_5_test]

    all_blurry_paths_train = []
    all_gt_paths_train = []
    all_labels_train = []

    all_blurry_paths_val = []
    all_gt_paths_val = []
    all_labels_val = []

    all_blurry_paths_test = []
    all_gt_paths_test = []
    all_labels_test = []

    for data_idx in range(len(gopro_0_train)):
        torch.cuda.empty_cache()  # Free up any cached memory
        blurry_paths, gt_paths, labels = process_dataset(gopro_mix_train, data_idx, 'train')
        all_blurry_paths_train.append(blurry_paths)
        all_gt_paths_train.append(gt_paths)
        all_labels_train.append(labels)

    for data_idx in range(len(gopro_0_val)):
        torch.cuda.empty_cache()  # Free up any cached memory
        blurry_paths, gt_paths, labels = process_dataset(gopro_mix_val, data_idx, 'val')
        all_blurry_paths_val.append(blurry_paths)
        all_gt_paths_val.append(gt_paths)
        all_labels_val.append(labels)

    for data_idx in range(len(gopro_0_test)):
        torch.cuda.empty_cache()  # Free up any cached memory
        blurry_paths, gt_paths, labels = process_dataset(gopro_mix_test, data_idx, 'test')
        all_blurry_paths_test.append(blurry_paths)
        all_gt_paths_test.append(gt_paths)
        all_labels_test.append(labels)

    save_output_to_file(all_blurry_paths_train, all_gt_paths_train, all_labels_train, 
                        all_blurry_paths_val, all_gt_paths_val, all_labels_val,
                        all_blurry_paths_test, all_gt_paths_test, all_labels_test,
                        output_file)
