...
class SearchTransfer(nn.Module):
    def __init__(self, n_feat=32):
        ...
        self.search2 = nn.Conv2d(n_feat *2, n_feat, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, lrsr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3):
        ...
        T_lv1 = F.fold(T_lv1_unfold, output_size=(lrsr_lv3.size(2)*4, lrsr_lv3.size(3)*4), kernel_size=(12,12), padding=4, stride=4) / (3.*3.)
            # print(T_lv3.size(), T_lv2.size(), T_lv1.size())

        S = R_lv3_star.view(R_lv3_star.size(0), 1, lrsr_lv3.size(2), lrsr_lv3.size(3))

        return S, T_lv3, T_lv2, T_lv1  #None, None  #

class SelfTransfer(nn.Module):
    def __init__(self, n_feat=32):
        super(SelfTransfer, self).__init__()
        self.search1 = nn.Conv2d(n_feat *4, n_feat*2, kernel_size=1, stride=1, padding=0)
        self.search2 = nn.Conv2d(n_feat *2, n_feat, kernel_size=1, stride=1, padding=0)
    
    def forward(self, lrsr_lv3):
        refsr_lv3 = lrsr_lv3.transpose(2, 3).flip(2)
        lrsr_lv3_unfold  = F.unfold(lrsr_lv3, kernel_size=(3, 3), padding=1)
        refsr_lv3_unfold = F.unfold(refsr_lv3, kernel_size=(3, 3), padding=1)
        refsr_lv3_unfold = refsr_lv3_unfold.permute(0, 2, 1)
        ...
        R_lv3 = torch.bmm(refsr_lv3_unfold, lrsr_lv3_unfold) #[N, Hr*Wr, H*W]
        R_lv3_star, _ = torch.max(R_lv3, dim=1) #[N, H*W]
       
        S = R_lv3_star.view(R_lv3_star.size(0), 1, lrsr_lv3.size(2), lrsr_lv3.size(3))
        T_lv3 = lrsr_lv3
        search_1 = F.interpolate(lrsr_lv3, scale_factor=2, mode='bicubic') #8,128,100,100
        T_lv2 = F.relu(self.search1(search_1)) #8,64,100,100
        search_2 = F.interpolate(T_lv2, scale_factor=2, mode='bicubic') #8,128,100,100
        T_lv1 = F.relu(self.search2(search_2)) #8,64,100,100
        S = R_lv3_star.view(R_lv3_star.size(0), 1, lrsr_lv3.size(2), lrsr_lv3.size(3))
