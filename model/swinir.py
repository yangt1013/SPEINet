...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv_y = nn.Linear(dim, dim * 1, bias=qkv_bias)
    ...
    def forward(self, x, y, mask=None):
        ...
        """
        B_, N, C = x.shape
        # print(x.shape)  #128 64 512
        # qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_x = self.qkv_x(x).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv_y = self.qkv_y(y).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q_y, k_x, v_x = qkv_y[0], qkv_x[0], qkv_x[1]  # make torchscript happy (cannot use tensor as tuple)

        q_y = q_y * self.scale
        attn = (q_y @ k_x.transpose(-2, -1))
        ...
        x = (attn @ v_x).transpose(1, 2).reshape(B_, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class SwinIR(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            y_first = self.conv_first(y)
            res = self.conv_after_body(self.forward_features(x_first, y_first)) + x_first
